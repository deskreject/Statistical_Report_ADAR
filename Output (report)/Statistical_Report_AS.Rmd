...
In summary, model selection can't be driven entirely by fit parameters, but has to be guided by the researchers understanding of the data being analyzed as well as by the suitability of pretrends to allow any causal interpretation of diff-in-diff coefficients. What follows are an investigation of the error distribution as well as predictive ability of the count regression models to offer further insights for choosing the best model.  

#### Preconditions

Below, the qq plot for the residuals as well as the comparison of fitted vs actual answers of the logged DV models are presented to illustrate non-normality (and hence untrustworthyness of standard errors). 
...
```
inspecting the rootograms above, it can be observed that the hurdle models display less fitting errors, while the negative binomial versions perform better than the poisson model. Both is no surprise given the combination of 0 inflation and overdispersion in the distribution of the dv of interest. 
...
An investigation of the outliers for 2 of the models, the OLS with the logged dependent variable as well as the negative binomial regression, yielded at least 2 users, 1 from each of the communities, that displayed unusually high values of answers, leading them to be flagged as outliers according to studentized residual and cook's distance values. 
...
#negative binomial with fixed effects --> don't work, can't estimate SEs. need to opt for poisson
pretrend_pois_FEs_m_unalt_asw_6m <- fepois(actions ~ group*as.factor(t_shock_w) + questions_per_week|as.factor(user_ID), data = w_panel_matched_qual_6m_unalt_b)
...
#plot the pretrends with FEs --> not possible as there is no way to get tidy data for fixest object + one of the periods doesn't have an estimation...
...
- the pretrends: Is the way I am assessing the parallel trends assumption the only viable way? If the count regression model is the best fitting model but the pretrends as shown aren't parallel, does this mean that I can't refer back to the findings that pretrends are parallel in the logged DV OLS specification?
