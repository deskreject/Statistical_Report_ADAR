...
The above plots give a summary of the distribution of the depenedent variable to assess which model is the best choice given the distribution. The Ord Plot, which plots the number of occurrences against a frequency ratio, suggest that a logarithmic series might fit the DV distribution best. However the ord plot is sensitive to single outlier values, which is why dist plots provide better indications for which distribution the variable may follow (see Friendly, 2000). Checking these plots as well as the likelihood provided for both the poisson and negative binomial, it is more likely that the variable follows a negative binomial distribution. 

### Model building and testing

To test the hypothesis of this research, that the introduction of symbolic awards has a negative effect on the asnwering behavior of members, a difference in difference design is employed, which translates into the following regression equation:
...
The coefficients of the count regression are a little harder to interpret, as it wasn't possible to specify time and individual fixed effects, thus inclusion or exclusion of observations was not made based on the user level, leading to a mean comparison of the two groups across two time periods, not accounting for user level panel dynamics. 
...
Below, the qq plot for the residuals as well as the comparison of fitted vs actual answers of the logged DV models are presented to illustrate non-normality (and hence untrustworthiness of standard errors). 
...
#plot them
plot(logged_fitted_values,logged_answers_sample , xlim = c(0,3))
...
infIndexPlot(ols_logged_matched_unalt_asw_6m) #--> ID: 647540000 identified
infIndexPlot(negbi_matched_unalt_asw_6m) #--> ID: 647540000 identified again
```
...
Below, the point estimates and standard errors for the $\beta_{2}$ are presented for the poisson regression with fixed effects specification - the most suitable model given the data structure aside from the negative binomial specification. It has to be said that it was not possible to estimate pretrends for the negative binomial models with fixed effects or the hurdle models as they did not converge - the implications of this is currently not clear to the authors. In addition, the pretrends using the OLS model with the logged DV variable, including full fixed effects and errors clustered at the individual level are presented
...
For optimal pretrends one would want the confidence intervals in each period to cross the horizontal line at y=0, implying that statistically, difference in the change from period to period in each group is equal to 0 (see Barbosu and Gans, 2017). As is evident from the pretrend analysis above using the poisson regression with fixed effects on the week and individual level specification, pretrends are not optimal when comparing the two groups, raising considerable doubt about whether the difference-in-difference coefficient can be interpreted as representing a causal effect of the symbolic award introduction. 
...
A number of issues can be listed, both from a data and performance analysis standpoint. A large issue concerning the data is, taht we cannot confidently claim that some other event which occured before the treatment introduction caused the decline in answers of the treatment group as of this moment. Furthermore, it transpired that the distribution of individual answering behavior in either community is quite different. While in both communities, answering behavior demonstrates power law properties, in that there are a very large number of barely active users and a very small number of highly active users, this effect seems to be especially pronounced in the treated community. We tried to adjust for this using matching procedures, however it seems that while pretrends did impreove compared to the unmatched data, the improvement isn't sufficient to assert parallel trends before treatment for both communities
