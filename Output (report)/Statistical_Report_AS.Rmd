...
if (!require(pscl)) install.packages("pscl"); library(pscl) #for bootstrapped standard errors
...
- need to preface that I will be using count regressions as the main model (if I can make them work) given the non-normal, count distribution of the data being investigated. 
...
#OLS regressions
ols_unmatched_asw_6m <- lm(formula_reg, data = w_panel_did_qual_as_regUs_6m_b)
ols_logged_unmatched_asw_6m <- lm(formula_log_reg, data = w_panel_did_qual_as_regUs_6m_b)
ols_logged_fullFEs_asw_6m <- felm(log(actions + 1) ~  group*post_treatment + questions_per_week|user_ID + as.factor(t_shock_w)|0|user_ID, data = w_panel_did_qual_as_regUs_6m_b)

#run simple poisson without fixed effects
pois_unmatched_asw_6m <- glm(actions ~ group*post_treatment + questions_per_week, family = poisson, data = w_panel_did_qual_as_regUs_6m_b)
#run the poisson model with time and individual fixed effects
pois_unmatched_FEs_asw_6m <- fepois(actions ~ group*post_treatment + questions_per_week|as.factor(user_ID) + as.factor(t_shock_w), w_panel_did_qual_as_regUs_6m_b)
pois_m_unalt_FEs_asw_6m <- fepois(actions ~ group*post_treatment + questions_per_week|as.factor(user_ID) + as.factor(t_shock_w), w_panel_matched_qual_6m_unalt_b)

## overdispersed poisson models
#test for dispersion
(phihat <- sum(residuals(pois_unmatched_asw_6m, type = "pearson") ^ 2) / df.residual(pois_unmatched_asw_6m))
dispersiontest(pois_unmatched_asw_6m)

#summary of the poisson with overdispersion adjustment
summary(pois_unmatched_asw_6m, dispersion = phihat)
summary(pois_unmatched_FEs_asw_6m, dispersion = phihat)


#negative binomial regressions
##first need to add the did_coeff to avoid multicollinearity
w_panel_did_qual_as_regUs_6m_b$did_coeff <- w_panel_did_qual_as_regUs_6m_b$group*w_panel_did_qual_as_regUs_6m_b$post_treatment 
#with fixed effects
negbi_unmatched_FEs_asw_6m_1 <- fenegbin(actions ~ did_coeff + questions_per_week|as.factor(user_ID) + as.factor(t_shock_w), w_panel_did_qual_as_regUs_6m_b)

#without fixed effects
(theta_unmatched <- glm.nb(actions ~ group*post_treatment + questions_per_week, data = w_panel_did_qual_as_regUs_6m_b)$theta) 
negbi_unmatched_FEs_asw_6m_2 <- update(pois_unmatched_asw_6m, family = negative.binomial(theta_unmatched))

#show the model results
summary(negbi_unmatched_FEs_asw_6m_1)
summary(negbi_unmatched_FEs_asw_6m_2)


#hurdle models
#negbi - no FEs
negbi_h_unmatched_asw_6m <- hurdle(actions ~ group*post_treatment + questions_per_week, data = w_panel_did_qual_as_regUs_6m_b, dist = "negbin")
#poisson - no FEs
pois_h_unmatched_asw_6m <- hurdle(actions ~ group*post_treatment + questions_per_week, data = w_panel_did_qual_as_regUs_6m_b)

summary(negbi_h_unmatched_asw_6m)

#zero inflated poisson
#negbi - no FEs
negbi_zi_unmatched_asw_6m <- zeroinfl(actions ~ group*post_treatment + questions_per_week, data = w_panel_did_qual_as_regUs_6m_b, dist = "negbin")
#poisson - no FEs
pois_zi_unmatched_asw_6m <- zeroinfl(actions ~ group*post_treatment + questions_per_week, data = w_panel_did_qual_as_regUs_6m_b)

summary(pois_zi_unmatched_asw_6m)

#show the models
etable(negbi_unmatched_FEs_asw_6m_1,
       pois_unmatched_FEs_asw_6m)

stargazer(ols_unmatched_asw_6m,
          ols_logged_unmatched_asw_6m,
          ols_logged_fullFEs_asw_6m,
          negbi_unmatched_FEs_asw_6m_2,
          pois_unmatched_asw_6m,
          negbi_h_unmatched_asw_6m,
          pois_h_unmatched_asw_6m,
          type = "text")
...
pois_h_matched_unalt_FEs_asw_6m <- hurdle(actions ~ did_coeff + questions_per_week + as.factor(t_shock_w), data = w_panel_matched_qual_6m_unalt_b)
# negbi hurdle - with dummies for time fixed effects
negbi_h_matched_unalt_FEs_asw_6m <- hurdle(actions ~ did_coeff + questions_per_week + as.factor(t_shock_w), data = w_panel_matched_qual_6m_unalt_b, dist = "negbin")
...
stargazer(ols_matched_unalt_asw_6m,
          ...
          type = "text")
```
Interpretation from the 0 inflated/hurdle models:
...
pretrend_asw_matched_unalt_pois <- glm(actions ~ group*as.factor(t_shock_w) + questions_per_week, family = poisson, data = w_panel_matched_qual_6m_unalt_b) 
(theta_pretrend <- glm.nb(actions ~ group*as.factor(t_shock_w) + questions_per_week, data = w_panel_matched_qual_6m_unalt_b)$theta) 
pretrend_asw_matched_unalt_negbi <- update(pretrend_asw_matched_unalt_pois, family = negative.binomial(theta_pretrend))


#plot the pretrends
fixest::coefplot(pretrend_asw_matched_unalt_negbi, keep = ":",
                                                            col = 4, 
...
For optimal pretrends one would want the confidence intervals in each period to cross the horizontal line at y=0, implying that statistically, difference in the change from period to period in each group is equal to 0. As is evident from the pretrend analysis above using the negative binomial specification, pretrends are not optimal when comparing the two groups, raising considerable doubt about whether the difference-in-difference coefficient can be interpreted as representing the causal effect of the symbolic award introduction. 
...
tidy_log_pretrends %>% filter(is.na(estimate) == F, term != "questions_per_week") %>% ggplot(aes(x = term, y = estimate)) + 
  geom_point() + 
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high)) +
  geom_hline(yintercept = 0) + 
  geom_vline(xintercept = "group:as.factor(t_shock_w)0", color = "red")
  
```

The pretrends above, which are based on the model specification using the logged dependent variable, including indivdual and time fixed effects as well as clustering the standard errors on the individual, while not perfect, provide better arguments for interpreting the difference in difference coefficient of the model as causal. 

In summary, while the distribution of the dependent variable suggests that a count model, specifically one that accounts for overdispersion, should be used, the pretrends do not allow for the causal interpretation of the DiD coefficient. Taking into account that the pretrends look more promising when using the logged DV specification for the pretrend analysis, in addition to the fact that the DiD coefficient is negative and significant in every model specification, the conclusion is drawn that the introduction of the symbolic awards leads to a reduction in the average answers per user. 
